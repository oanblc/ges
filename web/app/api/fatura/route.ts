...
/** Fatura ayıklama ucu — asistan servisine düz JSON proxy'si. */
export async function POST(req: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${SERVIS}/fatura-ayikla`, {
      method: "POST",
    ...
    return Response.json(
      { hata: "Fatura okuma servisi şu an çalışmıyor; lütfen daha sonra deneyin." },
      { status: 503 },
    );
  }
