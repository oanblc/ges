...
/** Saatlik tüketim analizi ucu — asistan servisine düz JSON proxy'si. */
export async function POST(req: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${SERVIS}/saatlik-analiz`, {
      method: "POST",
    ...
    return Response.json(
      { hata: "Analiz servisi şu an çalışmıyor; lütfen daha sonra deneyin." },
      { status: 503 },
    );
  }
